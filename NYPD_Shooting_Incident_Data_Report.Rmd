...
This incident data report will provide specific analysis and results, based upon the following dataset: 'NYPD Shooting Incident Data (Historic)'. To get started, we will need to install the following R libraries for further use:

1.) tidyverse
2.) sessioninfo

Once installed, the following code will load key packages from the required library that will be used for this analysis.
...
2.) Right click on the CSV button next to the title, and copy the link address into your respective RMD document, as shown in the code below.
...
ggplot(df_w_pred)+
  geom_point(aes(x=brooklyn_dates, y=brooklyn_precincts), color="darkblue")+
  geom_point(aes(x=brooklyn_dates, y=pred), color="green")+
